...
export SUBDOMAIN_ZONE_ID=$(aws route53 list-hosted-zones | jq -r --arg SUBDOMAIN "$CLUSTER.twdps.io." '.HostedZones[] | select(.Name==$SUBDOMAIN) | .Id')
...
export PARENT_ZONE_ID=$(aws route53 list-hosted-zones | jq -r '.HostedZones[] | select(.Name=="twdps.io.") | .Id')
echo $PARENT_ZONE_ID
echo "Creating NS records for $CLUSTER.twdps.io in parent zone"
