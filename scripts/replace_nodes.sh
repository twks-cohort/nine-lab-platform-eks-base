...
#         # INSTANCE_ID=$(aws ec2 describe-instances --filter Name=private-dns-name,Values=$i --profile PlatformEksRole | jq -r '.Reservations[].Instances[] | .InstanceId')
#         # aws ec2 terminate-instances --instance-ids ${INSTANCE_ID} --profile PlatformEksRole
